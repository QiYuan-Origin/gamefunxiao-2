...
public class PlayerListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        plugin.getGameManager().handleFlashPreyStartFireworkUse(event, player, room);

        if (plugin.getGameManager().handleFlashPreyStartElytraProtectedInteract(event, player, room)) {
    ...
    private boolean handleLobbyItem(Player player, GameRoom room, ItemStack item, Material type, int modelData, RoomState state) {
        ...
        if (plugin.getGameManager().isDeathSwapTimeVoteItem(item)) {
    ...
    private boolean handleGameItem(Player player, GameRoom room, Material type, int modelData) {
        ...
        if (plugin.getGameManager().handleFlashPreyStartChoiceUse(player, room, modelData)) {
            ...
            if (isTeamTeleportCompassUser(player, room, item)) {
                ...
                if (lastCompassDrop.containsKey(uuid) && now - lastCompassDrop.get(uuid) < 2000) {
                    ...
                    if (count >= 2) {
                        ...
                        openHunterTeleportMenu(player, room, false);
    ...
    public void onPlayerRespawn(org.bukkit.event.player.PlayerRespawnEvent event) {
        ...
        org.bukkit.Location vanillaRespawnLoc = getVanillaRoomRespawnLocation(event, room);

        if (vanillaRespawnLoc != null && room.getGameMode() != GameMode.END_FLASH) {

            event.setRespawnLocation(vanillaRespawnLoc);

            Bukkit.getScheduler().runTaskLater(plugin,

                    () -> plugin.getGameManager().syncEndDimensionBrightness(player, room), 2L);

            applyFlashRespawnProtectionAndCooldown(player, room, vanillaRespawnLoc);
        ...
        if (pendingLoc != null) {
            ...
            applyFlashRespawnProtectionAndCooldown(player, room, pendingLoc);
        ...
        if (room.getState() == RoomState.PLAYING) {
            ...
            org.bukkit.Location eventRespawnLoc = getRoomEventRespawnLocation(event, room);

            if (eventRespawnLoc != null) {

                event.setRespawnLocation(eventRespawnLoc);

                Bukkit.getScheduler().runTaskLater(plugin,

                        () -> plugin.getGameManager().syncEndDimensionBrightness(player, room), 2L);

                applyFlashRespawnProtectionAndCooldown(player, room, eventRespawnLoc);
            ...
            if (respawnLoc != null && respawnLoc.getWorld() != null) {
                ...
                if (respawnRoomId != null && respawnRoomId.equals(room.getRoomId())) {
                    ...
                    org.bukkit.Location safeRespawn = getSafeRespawnLocation(respawnLoc);
                    ...
                    applyFlashRespawnProtectionAndCooldown(player, room, safeRespawn);
            ...
            if (respawnLoc != null && room.getGameWorld() != null && respawnLoc.getWorld() != null
                ...
                org.bukkit.Location safeRespawn = getSafeRespawnLocation(room.getGameWorld().getSpawnLocation());
    ...
    private org.bukkit.Location getVanillaRoomRespawnLocation(org.bukkit.event.player.PlayerRespawnEvent event, GameRoom room) {

        if (event == null || room == null || (!event.isBedSpawn() && !event.isAnchorSpawn())) {

            return null;

        }

        return getRoomEventRespawnLocation(event, room);

    }



    private org.bukkit.Location getRoomEventRespawnLocation(org.bukkit.event.player.PlayerRespawnEvent event, GameRoom room) {
        ...
        org.bukkit.Location respawnLocation = event.getRespawnLocation();
        ...
        String respawnRoomId = plugin.getWorldManager().getRoomIdByWorld(respawnLocation.getWorld());
        ...
        return centerRespawnLocation(respawnLocation);

    }



    private org.bukkit.Location centerRespawnLocation(org.bukkit.Location location) {
        ...
        org.bukkit.Location centered = location.clone();
    ...
    private void applyFlashRespawnProtectionAndCooldown(Player player, GameRoom room, org.bukkit.Location respawnLocation) {
        ...
        if (isPreyNearRespawnLocation(room, respawnLocation, 250.0D)) {
    ...
    private boolean isPreyNearRespawnLocation(GameRoom room, org.bukkit.Location respawnLocation, double radius) {

        if (room == null || respawnLocation == null || respawnLocation.getWorld() == null || radius <= 0.0D) {
        ...
        for (UUID preyId : room.getPreyUUIDs()) {

            Player prey = Bukkit.getPlayer(preyId);

            if (prey == null || !prey.isOnline() || prey.isDead()) {

                continue;

            }

            org.bukkit.Location preyLocation = prey.getLocation();

            if (preyLocation.getWorld() != null

                    && preyLocation.getWorld().equals(respawnLocation.getWorld())

                    && preyLocation.distanceSquared(respawnLocation) <= radiusSquared) {
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (vanillaDeathMessage != null) {
            ...
            Component message = room.getGameMode().isFlashTournament()
                    ...
                    ? withDeathSwapPrefix(vanillaDeathMessage)
        ...
        if (plugin.getGameManager().handleDeathSwapDeath(event, player, room)) {
            ...
            if (room.hasModifier("HunterTPOnDeath")) {
                ...
                if (hunterCount >= 2) {
                    ...
                    Bukkit.getScheduler().runTaskLater(plugin, () -> {
                        ...
                        openHunterTeleportMenu(player, room, true);
    ...
    public void onPlayerAdvancementDone(PlayerAdvancementDoneEvent event) {
        ...
        if (room == null) {
            ...
            if (isGameFunFlashAdvancement(event.getAdvancement())) {

                event.message(null);

                Bukkit.broadcast(withHunterGamePrefix(advancementMessage));
        ...
        Component roomMessage = room.getGameMode().isFlashTournament()
                ...
                ? withDeathSwapPrefix(advancementMessage)
    ...
    private Component withDeathSwapPrefix(Component message) {

        return LegacyComponentSerializer.legacySection()

                .deserialize(plugin.getConfigManager().getDeathSwapPrefix() + "§r")
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        recordHunterGameMiningContribution(event, player, room);

    }



    private boolean isGameFunFlashAdvancement(org.bukkit.advancement.Advancement advancement) {
        ...
        return "gamefunxiao".equalsIgnoreCase(advancement.getKey().getNamespace())

                && advancement.getKey().getKey().toLowerCase(Locale.ROOT).startsWith("flash/");

    }


    private void recordHunterGameMiningContribution(BlockBreakEvent event, Player player, GameRoom room) {

        if (event == null || event.isCancelled() || player == null || room == null

                || room.getState() != RoomState.PLAYING

                || !room.getGameMode().usesHunterFlowMode()
        ...
        int value = getPerformanceMiningValue(event.getBlock().getType());

        if (value > 0) {

            room.addMiningContribution(player.getUniqueId(), value);
    ...
    private int getPerformanceMiningValue(Material material) {
        ...
        if (name.contains("IRON_ORE") || name.contains("COPPER_ORE")) {

            return 3;

        }

        if (name.contains("COAL_ORE") || name.contains("NETHER_QUARTZ_ORE")) {
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (room.getState() == RoomState.PLAYING) {
            ...
            if (room.getGameMode().isDeathSwap()) {
                ...
                plugin.getGameManager().handleDeathSwapDamage(event, attacker, victim, room);
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (plugin.getGameManager().handleFlashPreyStartElytraInventoryClick(event, player, room)) {
    ...
    public void onInventoryDrag(InventoryDragEvent event) {
        ...
        if (plugin.getGameManager().handleFlashPreyStartElytraInventoryDrag(event, player, room)) {
    ...
    public void onPlayerItemDamage(PlayerItemDamageEvent event) {
        ...
        plugin.getGameManager().handleFlashPreyStartElytraDamage(event);
    ...
    public void onPlayerPortal(PlayerPortalEvent event) {
        ...
        if (targetWorld != null) {
            ...
            if (event.getCause() == PlayerTeleportEvent.TeleportCause.NETHER_PORTAL) {
                ...
                org.bukkit.Location targetLoc = event.getTo();
                ...
                if (targetLoc == null) {
                    ...
                    double x = fromWorld.getEnvironment() == World.Environment.NORMAL ? from.getX() / 8.0D : from.getX() * 8.0D;

                    double z = fromWorld.getEnvironment() == World.Environment.NORMAL ? from.getZ() / 8.0D : from.getZ() * 8.0D;
                ...
                if (event.getFrom() != null) {
                    ...
                    targetLoc.setYaw(event.getFrom().getYaw());

                    targetLoc.setPitch(event.getFrom().getPitch());

                }



                portalLocationCache.remove(player.getUniqueId());
            ...
            if (event.getCause() == PlayerTeleportEvent.TeleportCause.END_PORTAL) {
                ...
                if (fromWorld.getEnvironment() == World.Environment.NORMAL) {
                    ...
                    // 主世界进末地：回到原版那块黑曜石出生平台

                    targetLoc = findVanillaEndPortalArrivalLocation(targetWorld, event.getFrom());
                    ...
                    if (room.getGameMode().isDeathSwap()) {
                        ...
                        targetLoc = center != null && center.getWorld() != null

                                ? center.clone()

                                : targetWorld.getSpawnLocation().clone();

                    } else {

                    // 末地返回主世界：继续使用安全落点，避免把玩家丢进虚空或危险地形

                    targetLoc = findSafeEndPortalReturnLocation(targetWorld, event.getFrom());
    ...
    private Location findVanillaEndPortalArrivalLocation(World world, Location reference) {
        ...
        float yaw = reference == null ? world.getSpawnLocation().getYaw() : reference.getYaw();

        float pitch = reference == null ? world.getSpawnLocation().getPitch() : reference.getPitch();

        ensureVanillaEndObsidianPlatform(world);

        return new Location(world, 100.5D, 51.0D, 0.5D, yaw, pitch);

    }



    private void ensureVanillaEndObsidianPlatform(World world) {
    ...
    private Location findEndObsidianPlatformSpawnLocation(World world, Location anchor, float yaw, float pitch) {
        ...
        int minX = anchor.getBlockX() - 24;

        int maxX = anchor.getBlockX() + 24;

        int minZ = anchor.getBlockZ() - 24;

        int maxZ = anchor.getBlockZ() + 24;

        int minY = Math.max(world.getMinHeight() + 1, anchor.getBlockY() - 18);

        int maxY = Math.min(world.getMaxHeight() - 3, anchor.getBlockY() + 18);
        ...
        for (int x = minX; x <= maxX; x++) {
            ...
            for (int z = minZ; z <= maxZ; z++) {
                ...
                for (int y = minY; y <= maxY; y++) {
                    ...
                    Location candidate = new Location(world, x + 0.5D, y + 1.0D, z + 0.5D, yaw, pitch);

                    double distance = candidate.distanceSquared(anchor);
    ...
    private Location findSafeEndPortalReturnLocation(World world, Location reference) {
        ...
        float yaw = reference == null ? spawn.getYaw() : reference.getYaw();

        float pitch = reference == null ? spawn.getPitch() : reference.getPitch();
        ...
        for (int radius = 24; radius <= 128; radius += 8) {
            ...
            for (int step = 0; step < 16; step++) {
                ...
                Location candidate = findEndFlashSurfaceLocation(world, x, z, yaw, pitch);

                if (candidate != null && candidate.distanceSquared(spawn) >= 576.0D) {
        ...
        for (int radius : new int[]{144, 176, 224}) {

            for (int step = 0; step < 12; step++) {

                double angle = (Math.PI * 2.0D * step) / 12.0D;
    ...
    private org.bukkit.Location getSafeRespawnLocation(org.bukkit.Location location) {
        ...
        if (isVanillaSafeRespawnLocation(world, x, y, z)) {
    ...
    private boolean isVanillaSafeRespawnLocation(World world, int x, int y, int z) {
        ...
        return floor.isSolid() && !feet.isSolid() && !head.isSolid();
    ...
    public void onEntityDeath(org.bukkit.event.entity.EntityDeathEvent event) {
        ...
        // 非终章模式不再拦截龙池传送门；击杀末影龙立即按原猎人游戏规则结算猎物胜利。
    ...
    public boolean openHunterTeleportMenu(Player player, GameRoom room, boolean mandatoryRespawnSelection) {
        ...
        try {
            ...
            Class<?> menuClass = Class.forName("org.gamefunxiao.menu.hunter.TeleportTeammateMenu",

                    true, plugin.getClass().getClassLoader());

            Object menu = menuClass

                    .getConstructor(GameFunXiao.class, Player.class, GameRoom.class, boolean.class)

                    .newInstance(plugin, player, room, mandatoryRespawnSelection);

            if (menu instanceof org.gamefunxiao.menu.base.BaseMenu baseMenu) {
            ...
            throw new IllegalStateException("TeleportTeammateMenu 不是 BaseMenu");

        } catch (ReflectiveOperationException | LinkageError | RuntimeException ex) {

            plugin.getLogger().warning("猎人队友传送菜单加载失败，已安全拦截本次打开: "

                    + ex.getClass().getSimpleName() + ": " + ex.getMessage());

            if (player.isOnline()) {

                player.playSound(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 0.75F, 0.65F);

                player.sendMessage(plugin.getConfigManager().getHunterGamePrefix()

                        + "§x§F§F§8§8§5§5⚠ §c队友传送菜单还没被当前子服加载成功，§e请重启对应子服加载最新插件后再试。");
